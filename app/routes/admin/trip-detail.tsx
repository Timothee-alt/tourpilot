...
const TripDetail = ({ loaderData }: Route.ComponentProps) => {
    ...
    const visitTimeAndWeatherInfo = [
        { title: 'Meilleur moment pour visiter:', items: bestTimeToVisit },
        { title: 'Météo:', items: weatherInfo }
    ]

    return (
        <main className="travel-detail wrapper">
            <Header title="Détails voyages" description="Voir et modifier les voyages générés par l'IA." />
            <section className="container wrapper-md">
                ...
                <ul className="itinerary">
                    {itinerary ?.map((dayPlan: DayPlan, index: number) => (
                        <li key={index}>
                            <h3>
                                Jour {dayPlan.day}: {dayPlan.location}
                            </h3>
            ...
            <section className="flex flex-col gap-6">
                ...
                <div className="trip-grid">
                    {allTrips.map(({id, name, location, imageUrls, itinerary, interests, travelStyle, estimatedPrice}) => (
                        <TripCard
                            id={id}
                            name={name}
